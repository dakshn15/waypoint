...
const CATEGORY_CONFIG: Record<string, { icon: React.ReactNode; color: string }> = {
  HOTEL: { icon: <Hotel className="h-4 w-4" />, color: "bg-secondary/10 text-secondary border-secondary/20" },
  TRANSPORT: { icon: <Car className="h-4 w-4" />, color: "bg-secondary/10 text-secondary border-secondary/20" },
  RESTAURANT: { icon: <Utensils className="h-4 w-4" />, color: "bg-primary/10 text-primary border-primary/20" },
  ACTIVITY: { icon: <Camera className="h-4 w-4" />, color: "bg-orange-500/10 text-orange-600 border-orange-500/20" },
  GUIDE: { icon: <Globe className="h-4 w-4" />, color: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" },
  OTHER: { icon: <Building2 className="h-4 w-4" />, color: "bg-slate-100 text-slate-500 border-slate-200" },
};
  ...
  return (
    <>
      ...
      <Dialog open={!!editTarget} onOpenChange={(open) => !open && setEditTarget(null)}>
        <DialogContent className="sm:max-w-md bg-white border border-slate-200 rounded-2xl p-6 shadow-2xl">
          <DialogHeader>
